...
//  [xmppStream connectToHost:@"localhost" onPort:0 withVirtualHost:@"localhost"];
//	NSString* hostname = @"";
//	NSUserDefaults* userDefaults = [NSUserDefaults standardUserDefaults];
//	NSString* domainName = [userDefaults objectForKey:EXO_PREFERENCE_DOMAIN];
//	NSRange tmpRange1 = [domainName rangeOfString:@"http://"];
//	for (int i = tmpRange1.location + tmpRange1.length; i < [domainName length]; i++)
//	{
//		if([domainName characterAtIndex:i] == ':')
//		{
//			hostname = [domainName substringWithRange:NSMakeRange(tmpRange1.location + tmpRange1.length, i - tmpRange1.location - tmpRange1.length)];
//			break;
//		}
//		
//		if([domainName characterAtIndex:i] == '/')
...
//	}
	...
	NSString* tmp = self.domain;
	NSRange r1 = [tmp rangeOfString:@"http://"];
	if (r1.length > 0) 
	{
		tmp = [tmp stringByReplacingOccurrencesOfString:@"http://" withString:@""];
	}
	[xmppStream connectToHost:tmp onPort:port withVirtualHost:tmp];
